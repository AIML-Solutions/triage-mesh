...
- **Red-team suite** (`tests/redteam/test_attacks.py`, 13 scenarios): attacks 01–04 (T1 injection via advisory text, including delimiter escape), 05 (T2 hostile manifests), 06 (T3 smuggled fields in tool results), 07–09 (T4 cross-role calls, out-of-scope reads, rate-ceiling breach), 10 (T5 lateral token reuse), 11 (T6 exfil-shaped queries), 12 (T7 forged publication), 13 (T3 poisoned tool description — a server advertising a drifted manifest is refused before any call leaves the harness). All must fail closed for CI to pass.
- **Tool-manifest pins** (`scripts/pin_tools.py`, `tool_integrity` block in `deploy/policies.yaml`): SHA-256 over each first-party server's sorted (name, description, input schema) triples, computed in-process from the deployed code; `tests/test_integrity.py` fails CI if pins drift from the tree. This closes the T3 mitigation previously listed as deferred.
- **Harness regression gate** (`.github/workflows/ci.yml` job `harness`): the flagship's tool-call trajectory is recorded in-process (`scripts/record_trajectory.py`) and evaluated by multiclaw-harness against `tests/harness/golden_cases.jsonl` — expected sequence, forbidden actions, budgets, and outcome rubric must all pass.
